...
impl Paths {
    ...
    pub fn resolve(env: &Env) -> Result<Self> {
        ...
        let titles = env.path("XGDK_TITLES").or_else(|| {
            repo.as_ref()
                .map(|r| r.join("titles"))
...
/// mounts, then the binary's own neighbourhood (`/usr/bin/xgdk` next to
/// `/usr/lib/xgdk`, or `target/debug/deps/...` inside a checkout).
fn resolve_tree(env: &Env) -> Option<PathBuf> {
    ...
    candidates.push(PathBuf::from("/usr/lib/xgdk"));
    candidates
        .into_iter()
...
"#
    ...
    fn defaults_are_the_ones_the_shell_scripts_use() {
        ...
        assert_eq!(p.state_dir(), Path::new("/home/tester/.local/state/xgdk"));
        assert_eq!(p.cache_dir(), Path::new("/home/tester/.cache/xgdk"));
    ...
    fn every_variable_overrides_its_default() {
        ...
        assert_eq!(p.cache_dir(), Path::new("/mnt/cache/xgdk"));
        assert_eq!(
            p.runtime_capabilities_file().unwrap(),
            Path::new("/mnt/proton/xgdk-capabilities.txt")
        );
    }
    ...
    fn the_tree_is_found_the_same_way_from_a_package_an_appimage_and_a_checkout() {
        let packaged = Paths::resolve(
            &env(&[])
                .with_exe("/usr/bin/xgdk")
                .with_existing(["/usr/lib/xgdk/scripts"]),
        )
        .unwrap();
        ...
        let appimage = Paths::resolve(
            &env(&[("APPDIR", "/tmp/.mount_xgdkAB")])
                .with_exe("/tmp/.mount_xgdkAB/usr/bin/xgdk")
                .with_existing([
                    "/tmp/.mount_xgdkAB/usr/lib/xgdk/scripts",
                    "/tmp/.mount_xgdkAB/usr/lib/xgdk/titles",
                ]),
        )
        .unwrap();
        assert_eq!(
            appimage.titles_dir(),
            Some(Path::new("/tmp/.mount_xgdkAB/usr/lib/xgdk/titles"))
        );

        // A test binary lives three directories below the checkout root.
        let checkout = Paths::resolve(
            &env(&[])
                .with_exe("/home/tester/src/xgdk-launcher/target/debug/deps/xgdk_core-1234")
                .with_existing([
                    "/home/tester/src/xgdk-launcher/scripts",
                    "/home/tester/src/xgdk-launcher/titles",
                ]),
        )
        .unwrap();
        assert_eq!(
            checkout.tree_dir(),
            Some(Path::new("/home/tester/src/xgdk-launcher"))
        );
    }
