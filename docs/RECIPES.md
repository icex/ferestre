...
git clone https://github.com/<owner>/gdk-linux ~/src/gdk-linux

# apply the local patches (they are plain git diffs against the fork)
cd ~/src/xodus-proton/wine
git apply ~/src/gdk-linux/patches/wine/*.patch
git -C dlls/xgameruntime apply ~/src/gdk-linux/patches/xgameruntime/*.patch
...
~/src/gdk-linux/scripts/install-xodus-proton.sh
```
...
echo "WORKDIR=$PWD/obj-wine-x86_64 ~/src/gdk-linux/tools/in-container.sh \
