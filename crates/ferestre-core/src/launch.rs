...
pub struct Dirs {
    ...
    /// `usr/lib/ferestre/scripts` in a package. `proton-wine-shim.sh` is taken from
    /// here.
...
impl Plan {
    /// The value a variable will have, for tests and for an `ferestre env`-style
...
mod tests {
    ...
    fn dirs() -> Dirs {
        Dirs {
            ...
            scripts_dir: PathBuf::from("/usr/lib/ferestre/scripts"),
        }
    }
    ...
    fn the_program_is_the_client_and_never_the_title() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/games/example",
                "/usr/lib/ferestre/scripts/proton-wine-shim.sh",
            ]
        );
    }
