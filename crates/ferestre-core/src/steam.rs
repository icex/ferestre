...
mod tests {
    ...
    fn adding_a_shortcut_leaves_every_other_entry_untouched() {
        ...
        let shortcut = Shortcut::new(
            "Test Title",
            Path::new("/opt/ferestre/bin/ferestre"),
            Path::new("/opt/ferestre/bin"),
            "run 9ZZTESTGAME1",
        );
        assert!(
        ...
        assert_eq!(
            ...
            Some("\"/opt/ferestre/bin/ferestre\"")
        );
        assert_eq!(
    ...
    fn adding_the_same_title_twice_updates_and_keeps_their_settings() {
        ...
        for (key, value) in fields.iter_mut() {
            match key.as_str() {
                ...
                "tags" => *value = Value::Map(vec![("0".into(), Value::Str("ferestre".into()))]),
                _ => {}
            }
        }
    ...
    fn paths_are_quoted() {
        let shortcut = Shortcut::new(
            "Test Title",
            Path::new("/home/someone/My Games/ferestre"),
            Path::new("/home/someone/My Games"),
            "",
        );
        assert_eq!(shortcut.exe, "\"/home/someone/My Games/ferestre\"");
    ...
    fn the_app_id_is_derived_the_way_steam_derives_it() {
        let id = shortcut_app_id("\"/opt/ferestre/bin/ferestre\"", "Test Title");
        assert_eq!(id & 0x8000_0000, 0x8000_0000, "the high bit is always set");
        assert_eq!(
            id,
            shortcut_app_id("\"/opt/ferestre/bin/ferestre\"", "Test Title"),
            "stable"
        );
        assert_ne!(
            id,
            shortcut_app_id("\"/opt/ferestre/bin/ferestre\"", "Other Title")
        );
    }
