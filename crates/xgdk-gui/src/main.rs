...
use xgdk_core::{account, catalog, steam};
...
fn build_ui(app: &adw::Application) {
    ...
    sidebar_header.set_title_widget(Some(&adw::WindowTitle::new("xgdk", "")));
...
fn refresh_library(ui: &Ui) {
    ...
    glib::spawn_future_local(async move {
        let result = gio::spawn_blocking(move || {
            ...
            let entries = xgdk_core::library::parse(&String::from_utf8_lossy(&output.stdout))
