...
#
# build-appimage.sh installs this as usr/bin/xgdk only when no real binary is
# found. Point XGDK_BIN at target/release/xgdk and it is replaced.
#
# It also runs straight out of a git checkout:  packaging/appimage/xgdk-placeholder.sh doctor
...
#
# Inside the AppImage they are at $APPDIR/usr/lib/xgdk/scripts; in a checkout
...
for cand in \
    "${XGDK_SCRIPTS_DIR:-}" \
    "${APPDIR:-/nonexistent}/usr/lib/xgdk/scripts" \
    "$selfdir/../lib/xgdk/scripts" \
    "$selfdir/../../scripts"
do
...
if [ -z "${SCRIPTS:-}" ]; then
    echo "!! cannot find the xgdk scripts directory (looked for xodus-env.sh)" >&2
    echo "   set XGDK_SCRIPTS_DIR to the directory holding launch-gdk.sh" >&2
    exit 1
fi

# xodus-env.sh derives XODUS_REPO_DIR from $0, which would be *this* file and
# therefore wrong once the scripts live under usr/lib/xgdk. install-xodus-proton.sh
...
version() {
    if   [ -n "${XGDK_VERSION:-}" ];   then printf '%s\n' "$XGDK_VERSION"
...
usage() {
    cat <<EOF
xgdk $(version) — placeholder build
...
cmd_env() {
    ...
    ( . "$SCRIPTS/xodus-env.sh"; env | grep -E '^(XODUS_|XGR_|XGDK_)' | sort )
}
...
cmd_titles() {
    ...
    local recipes="${XGDK_TITLES:-$SCRIPTS/../titles}"
    if [ -d "$recipes" ] && ls "$recipes"/*.toml >/dev/null 2>&1; then
        ...
        printf '\nlaunch one with: xgdk run <product-id>\n'
        return 0
    fi
...
cmd_doctor() {
    ...
    if [ -n "$XODUS_PROTON_DIR" ] && [ -f "$XODUS_PROTON_DIR/proton" ]; then
        ...
        note "run: xgdk install-runtime   (or set XODUS_PROTON_DIR)"
    fi
    ...
    printf '\n'
    if [ "$fails" -eq 0 ]; then
        printf '%sready.%s  next: xgdk titles\n' "$C_OK" "$C_OFF"
    else
...
case "$cmd" in
    ...
    download|install)
        [ $# -ge 1 ] || { echo "usage: xgdk download <product-id> [destination]" >&2; exit 2; }
        require_script get-game.sh
        exec "$SCRIPTS/get-game.sh" "$@"
        ;;

    run)
        [ $# -ge 1 ] || { echo "usage: xgdk run <title>   (see: xgdk titles)" >&2; exit 2; }
    ...
    run-raw)
        [ $# -ge 3 ] || { echo "usage: xgdk run-raw <game dir> <exe path in package> <prefix dir>" >&2; exit 2; }
